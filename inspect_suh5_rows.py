...
for t in subs:
  try:
    ...
    rows=cur.execute(f"SELECT * FROM [{t}] WHERE ahak='2'").fetchall()
  except Exception as e: continue
  print('\n##',t,'rows',len(rows))
  for r in rows:
    vals={col:r[i] for i,col in enumerate(cols)}
    print({k:vals.get(k) for k in ['sno','sdan','acha','ayung','ayu','ahak','prim','dan','summa','s52','ex1','ex2']})
